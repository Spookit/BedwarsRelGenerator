...
public enum XSound {
	...
	private static Map<String,XSound> cachedSounds = new HashMap<>();
	public static Sound getSound(String name) {
		XSound sound = getXSound(name);
		if (sound != null) sound.bukkitSound();
		return null;
	}
	public static XSound getXSound(String name) {
		XSound cached = cachedSounds.get(name);
		if (cached != null) return cached;
		for (XSound s : values()) {
			for (String sound : s.versionDependentNames) {
				if (sound.equals(name)) {
					cachedSounds.put(sound, s);
					return s;
				}
			}
		}
